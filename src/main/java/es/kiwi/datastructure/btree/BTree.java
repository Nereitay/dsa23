package es.kiwi.datastructure.btree;
...
public class BTree {
    ...
    static class Node {
        ...
        @Override
        public String toString() {// 打印有效数组部分
    ...
    Node root;
    int t; // 树中节点最小度数
    int MIN_KEY_NUMBER; // 最小key数目
    int MAX_KEY_NUMBER; // 最大key数目
    ...
    private void doRemove(Node parent, Node node, int index, int key) {
        ...
        if (node.keyNumber < MIN_KEY_NUMBER) {
            // 调整平衡 case 5 删除后 key 数目 < 下限（不平衡） & 6 根节点
    ...
    private void balance(Node parent, Node x, int i) {
        ...
        if (left != null && left.keyNumber > MIN_KEY_NUMBER) {// case 5-1 左边富裕，右旋
            // a) 父节点中前驱key旋转下来
        ...
        if (right != null && right.keyNumber > MIN_KEY_NUMBER) {// case 5-2 右边富裕，左旋
            // a) 父节点中后继key旋转下来
